...
const GoodsForm = ({
  ...
  const checkForm = () => {
    if (
      ...
      good.article &&
      good.brand &&
      fileInputMaterial.current.value &&
      good.mainparameter.length > 0 &&
      good.recommendparameter.length > 0 &&
      good.parameter.length > 0 &&
      fileInputCarousel.current.files.length > 0 &&
      fileInputPdf.current.value
    ) {
  ...
  const submitForm = async (e: any) => {
    ...
    axios
      .post("https://api.atman-auto.ru/api/goods", formData, {
        headers: {
  ...
  return (
    <>
      ...
      <ModalAlert
        ...
        message={textAlert}
        alertConfirm={() => console.log("alert")}
      />
    </>
  );
};
