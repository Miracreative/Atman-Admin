...
const EditGood = () => {
  ...
  const [mainParametersArray, setMainParametersArray] = useState<number[]>(
    good?.mainparameter
  );

  const [recommendParametersArray, setRecommendParametersArray] = useState<
    number[]
  >(good?.recommendparameter);

  const [parametersArray, setParametersArray] = useState<number[]>(
    good?.mainparameter
  );
  ...
  const deleteEmptyAdvantages = (): void => {
    let newGood = good;
    newGood.advantages = newGood.advantages.filter(
      (item: string) => item !== ""
    );
    setGood((state: any) => ({ ...state, advantages: newGood.advantages }));
  };
  ...
  const submitForm = async (e: any) => {
    ...
    formData.append(
      "advantages",
      JSON.stringify(good.advantages).replace("[", "{").replace("]", "}")
    );
    formData.append(
      "mainParameter",
      JSON.stringify(good.mainparameter).replace("[", "{").replace("]", "}")
    );
    formData.append(
      "recommendparameter",
      JSON.stringify(good.recommendparameter)
        .replace("[", "{")
        .replace("]", "}")
    );
    formData.append(
      "parameter",
      JSON.stringify(good.parameter).replace("[", "{").replace("]", "}")
    );

    axios
      .put("https://api.atman-auto.ru/api/goods", formData, {
        headers: {
  ...
  imagesCarousel =
    fileInputCarousel?.current?.files?.length > 0
      ? imagesItemsFromUpload(fileInputCarousel)
      : good?.goodscarouselimages?.length > 0
      ? imagesItemsFromBackend(good.goodscarouselimages)
      : "нет картинок";
  ...
  imagePdf =
    fileInputPdf?.current?.value.length > 0
      ? fileInputPdf?.current?.value.replace(regular, "")
      : good.pdfurl
      ? good.pdfurl.replace(regular, "")
      : "Файл не выбран";

  let imageMaterial;

  imageMaterial =
    fileInputMaterial?.current?.value.length > 0
      ? fileInputMaterial?.current?.value.replace(regular, "")
      : good.imageurl
      ? good.imageurl.replace(regular, "")
      : "Файл не выбран";
    ...
    const inputsForAdv = advantages?.map((_, i: number) => {
      return (
        <input
          type="text"
          key={i}
          className={`input ${errors.advantages ? "input--error" : ""}`}
          value={advantages[i]}
          onChange={(e) => handleChangeAdvantages(e, i)}
        />
      );
    });
    return inputsForAdv;
  ...
  const onMainParameters = (id: number) => {
    ...
    let newArr = [
      ...newArray.mainparameter.slice(0, id),
      newItem,
      ...newArray.mainparameter.slice(id + 1, newArray.mainparameter.length),
    ];
    ...
    let newArr1 = [
      ...newFilterArray.slice(0, id),
      newItem1,
      ...newFilterArray.slice(id + 1, newFilterArray.length),
    ];
  ...
  const onRecommendParameters = (id: number) => {
    ...
    let newArr = [
      ...newArray.recommendparameter.slice(0, id),
      newItem,
      ...newArray.recommendparameter.slice(
        id + 1,
        newArray.recommendparameter.length
      ),
    ];
  ...
  const onParameters = (id: number) => {
    ...
    let newArr = [
      ...newArray.parameter.slice(0, id),
      newItem,
      ...newArray.parameter.slice(id + 1, newArray.parameter.length),
    ];
        ...
        return (
          <li key={id} className="filters__item">
            <div
              className="filters__check"
              onClick={() => {
                onRecommendParameters(id);
              }}
            >
              {good.recommendparameter[i] ? (
                <img src={checked} alt="checked" />
              ) : null}
            </div>
            <span className="filters__title">{title}</span>
      ...
      (item: { id: number; title: string }, i: number) => {
        ...
        return (
          <li key={id} className="parameters__item">
            <div
              className="parameters__check"
              onClick={() => {
                onParameters(id);
              }}
            >
              {good.parameter && good.parameter[i] ? (
                <img src={checked} alt="checked" />
              ) : null}
            </div>
    ...
    <>
      ...
      >
        <div className="create-goods__box">
          ...
          <div className="create-goods__image-wrapper">{imagesCarousel}</div>
        </div>
        <div className="create-goods__wrap">
          ...
          <label className="create-goods__label">
            <span> </span>
            <input
              className={`input ${errors.thickness ? "input--error" : ""}`}
              type="text"
              name="thickness"
          ...
          <label className="create-goods__label">
            <span> </span>
            <input
              className={`input ${errors.linertype ? "input--error" : ""}`}
              type="text"
              name="linertype"
          ...
          <label className="create-goods__label">
            <span> </span>
            <input
              className={`input ${errors.basetype ? "input--error" : ""}`}
              type="text"
              name="basetype"
              value={good.basetype}
              onChange={handleChange}
          ...
          <label className="create-goods__label">
            <span> </span>
            <input
              className={`input ${errors.dencity ? "input--error" : ""}`}
              type="text"
              name="dencity"
              value={good.dencity}
              onChange={handleChange}
          ...
          <label className="create-goods__label">
            <span> </span>
            <input
              className={`input ${errors.pcs ? "input--error" : ""}`}
              type="text"
              name="pcs"
              value={good.pcs}
              onChange={handleChange}
          ...
          <label className="create-goods__label">
            <span> </span>
            <input
              className={`input ${errors.heatResistance ? "input--error" : ""}`}
              type="text"
              name="heatresistance"
          ...
          <label className="create-goods__label sr-only">
            <span> </span>
            <input
              className={`input ${errors.typeGlue ? "input--error" : ""}`}
              type="text"
              name="typeglue"
              value={good.typeglue}
              onChange={handleChange}
        ...
        <div className="create-goods__wrap create-goods__wrap--advantages">
          ...
          {advantagesList}
          <button
            className="button button--plus button--red"
            onClick={(e) => handleAddAdvantages(e)}
          >
            +
          </button>
        </div>
        <div className="create-goods__wrap">
          <h3 className="create-goods__title  ">
            К каким группам товарам относится данный товар
          </h3>
          <SetContent
            process={process}
            component={renderMainParameters(filtersData)}
          />
        </div>
        <div className="create-goods__wrap">
          <h3 className="create-goods__title  ">
            К каким группам рекомендаций относится данный товар
          </h3>
          <SetContent
            process={process}
            component={renderRecommendParameters(recommendData)}
          />
        </div>
        <div className="create-goods__wrap">
          ...
          <div className="parameters">
            <SetContent
              process={process}
              component={renderParameters(parametersData)}
            />
          </div>
        </div>
        <div className="create-goods__btns">
          <button
            type="button"
            className="create-goods__btn button button--orange"
            onClick={() => onConfirmDelete(good.id, good.name)}
          >
            Удалить товар
          </button>
          <button
            type="button"
            className="create-goods__btn button"
            disabled={checkForm()}
            onClick={(e) => {
              submitForm(e);
            }}
      ...
      <ConfirmModal
        ...
        text1={targetConfirm.name}
        text2={""}
        showConfirm={showConfirm}
        setShowConfirm={setShowConfirm}
        actionConfirmed={() => removeGood(targetConfirm.id)}
      />
      <ModalAlert
        ...
        message={textAlert}
        alertConfirm={() => (del ? navigate("/goods") : console.log("edit"))}
      />
    </>
  );
};
