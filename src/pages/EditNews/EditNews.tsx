...
const EditNews = () => {
  ...
  const submitForm = async (e: any) => {
    ...
    axios
      .put("https://api.atman-auto.ru/api/news", formData, {
        headers: {
  ...
  images =
    fileInput?.current?.files?.length > 0
      ? imagesItemsFromUpload()
      : news?.imagessrc?.length > 0
      ? imagesItemsFromBackend(news.imagessrc)
      : "нет картинок";
  ...
  return (
    <>
      ...
      >
        <div className="create-news__box">
          ...
          <label className="create-news__label">
            ...
            <input
              className={`input ${errors.descr ? "input--error" : ""}`}
              type="text"
              name="descr"
              value={news.descr}
              onChange={handleChange}
          ...
          <div className="create-news__image-wrapper">{images}</div>
          <label className="create-knowlege__label create-knowlege__input">
            ...
            <span>
              {mainInput?.current?.value.length > 0
                ? mainInput?.current?.value.replace(regular, "")
                : news.main
                ? news.main.replace(regular, "")
                : "Файл не выбран"}
            </span>
        ...
        <div className="create-news__btns">
          <button
            type="button"
            className="create-news__btn button button--orange"
            onClick={() => onConfirmDelete(news.id, news.title)}
          >
            Удалить новость
          </button>
          <button
            type="button"
            className="create-news__btn button"
            disabled={checkForm()}
            onClick={(e) => {
              submitForm(e);
            }}
      ...
      <ConfirmModal
        ...
        actionConfirmed={() => removeNews(targetConfirm.id)}
      />
      <ModalAlert
        ...
        message={textAlert}
        alertConfirm={() => (del ? navigate("/news") : console.log("edit"))}
      />
    </>
  );
};
